<template>
  <Teleport to="body">
    <Transition name="modal">
      <div
        v-if="isOpen"
        class="fixed inset-0 z-50 flex items-center justify-center bg-ink/80 px-4 backdrop-blur-sm"
        @click.self="close"
      >
        <div class="ticket ticket-ruled w-full max-w-md p-7 pt-8">
          <div class="flex justify-between items-center mb-6">
            <div>
              <p class="eyebrow mb-1">Amend</p>
              <h2 class="font-display text-2xl font-bold text-cloud">Edit Item</h2>
            </div>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-5">
            <div>
              <label for="item-name" class="field-label">Name*</label>
              <input
                id="item-name"
                v-model="form.name"
                type="text"
            ...
            <div>
              <label for="item-quantity" class="field-label">Quantity</label>
              <input
                id="item-quantity"
            ...
            <div>
              <label for="item-details" class="field-label">Details</label>
              <textarea
                id="item-details"
            ...
            <div v-if="error" class="band band-error">
              {{ error }}
            </div>

            <div class="flex gap-3 justify-center pt-1">
              <button type="button" @click="close" class="btn-quiet">
                Cancel
              </button>
              <button type="submit" :disabled="isSubmitting" class="btn-gold flex-1">
  ...
  <Teleport to="body">
    <Transition name="modal">
      ...
      >
        <div class="ticket ticket-ruled w-full max-w-md p-7 pt-8">
          <div class="mb-6">
            <p class="eyebrow mb-1">Reassign</p>
            <h2 class="font-display text-2xl font-bold text-cloud">Move Item</h2>
          </div>

          <form @submit.prevent="handleMoveItem" class="space-y-5">
            <div>
              <label for="move-item-list" class="field-label">Move to list</label>
              <select
                id="move-item-list"
            ...
            <div v-if="moveError" class="band band-error">
              {{ moveError }}
            </div>

            <div class="flex gap-3 justify-center pt-1">
              <button type="button" @click="closeMoveModal" class="btn-quiet">
                Cancel
              </button>
              <button
                type="submit"
